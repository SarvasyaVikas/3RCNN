...
PCV1 = network.parallel_filters(16, 5)
PCV2 = network.parallel_filters(16, 5)
PCV3 = network.parallel_filters(32, 3)
PCV4 = network.parallel_filters(64, 3)
...
FC5[rank] = network.generate_layer(64, 16)
FC6[rank] = network.generate_layer(16, 4)
SF[rank] = network.generate_layer(4, 2)
...
for i in range(val):
	...
	print(i)
	print("f")
	(networkN, error) = FunctionalNetwork.DRCNN(Is1[i], As1[i], neuralNetworks[-1], losses[-1], neuralNetworks, rank)
	losses.append(error)
	print(error)
...
for i in range(val):
	...
	print(i)
	print("f")
	(networkN, error) = FunctionalNetwork.DRCNN(Is2[i], As2[i], neuralNetworks[-1], losses[-1], neuralNetworks, rank)
	losses.append(error)
	print(error)
