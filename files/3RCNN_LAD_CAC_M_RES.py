...
for epoch in range(2):
    beg = 0 if epoch == 0 else 60
    newlen = 60 if epoch == 0 else len(mult)
    for j in range(beg, newlen):
        ...
        for i in range(mult[j][1], mult[j][2] - 4):
            start = time.time()
            (losses, networkS, CACscore, step) = RCNN.VR(rank, networkS, Is1, As1, mult, j, i, alpha, losses, minval, maxval, FOV, CACscore, start, CACactual, scan_start, nn, step, f, "LAD")
