...
class process:
    ...
    def cut_scoring(cut, fov, ch = 0):
        ...
        for i in range(h):
            for j in range(w):
                val = cut[i,j]
                (factor, counted) = process.scoring(val, ch)
                if factor > raw:
                    raw = factor
                if counted:
                    pixels += 1
        pixAREA = process.area(fov)
        scaled = raw * pixels * pixAREA
        return scaled
...
class antibone:
    ...
    def detectExternal(image):
        ...
        calcium_tuple = cv2.findContours(thresh, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)
        calcium_lists = []
        calcium_areas = []
        calcium_com = []
        calcium_spots = calcium_tuple[0]
        ...
        for i in range(len(calcium_com)):
            if (top < calcium_com[i][0] < bottom) and (left < calcium_com[i][1] < right):
