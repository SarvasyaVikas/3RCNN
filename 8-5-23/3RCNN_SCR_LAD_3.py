...
from sets_LAD import SETS
from algorithm import algorithm
from processes_VIII import antibone as ab
from processes_XI import process
...
for loop in range(70):
    ...
    for loop1 in range(56):
        ...
        for i in range(mult[j][1], mult[j][2] - 4):
            ...
            scr = process.cut_scoring(cut, FOV, x, y, z)
            (h, w) = cut.shape[:2]
            lst.append(h * w)
            CACscore += scr
            print("i")
        ...
        rec = open("scoresCHY.csv", "a+")
        new = ["NGCT{}".format(mult[j][0]), "SCR_LAD", str("{},{},{}".format(x, y, z)), rank, CACscore, CACactual, perdiff]
