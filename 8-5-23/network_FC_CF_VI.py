...
class network:
    ...
    def forward_pass(layer, vals, forward, activation = 0, antiactiv = False):
        new = [] # 0.01 seconds per network run from rca-cf
        ...
        for i in range(len(forward)):
            ...
            for j in range(len(layer)):
                ...
                totI += added
            sign = -1 if totI < 1 else 1
            tot = abs(np.log(abs(totI))) * sign
            if activation == 0:
