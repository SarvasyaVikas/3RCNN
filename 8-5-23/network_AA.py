...
class network:
    ...
    def indiv_weight_reg(val):
        res = (val ** 2)
        if res < 1:
            res = abs(val)
        return res

    def forward_pass(layer, vals, forward, activation = 0, antiactiv = False, wr = False):
        new = []
        inactive = []
        layer_reg = []
        for i in range(len(forward)):
            ...
            for j in range(len(layer)):
                weight_reg += network.indiv_weight_reg(layer[j][0][i])
                added = (float(layer[j][0][i]) * float(vals[j])) + layer[j][1]
                tot += added
            tot = tot / len(layer)
            activ = max(activation * np.cbrt(tot), np.cbrt(tot))
            inactive.append(tot)
            new.append(activ)
            layer_reg.append(weight_reg)
        
        if antiactiv and wr:
            return (new, inactive, layer_reg)
        elif antiactiv:
            return (new, inactive)
        elif wr:
            return (new, layer_reg)
        else:
            return new

    def loss_pass(layer, fin_vals, forward, activation = 0):
        new = []
        for i in range(len(layer)):
            tot = 0
            for j in range(len(layer[i][0])):
                added = (float(layer[i][0][j]) * float(fin_vals[j]))
                tot += added
            tot += layer[i][1]
            tot = tot / len(layer[i][0])
            if activation == 0:
                activ = max(activation * np.cbrt(tot), np.cbrt(tot))
                new.append(activ)
    ...
    def mse(actuals, coords):
        ...
        for i in range(5):
            tot = []
            for j in range(4):
                added = (float(actuals[i][j]) - float(coords[i][j])) ** 2
                tot.append(added)
            loss.append(tot)
        return loss
    ...
    def mseINDIV(actuals, coords):
        loss = []
        for j in range(4):
            added = (float(actuals[j]) - float(coords[j])) ** 2
            loss.append(added)
        return loss
    ...
    def backprop(layer, vals, activation, alpha, FP):
        ...
        for i in range(len(layer)):
            ...
            for j in range(len(FP)):
                ...
                activ = max(0.1 * added, added)
                change = activ * alpha
                # layer[i][0][j] -= change
                # this is the prev determination
                optimizeSAVElst.append(change)

                # layer[i][0][j] = network.betw(layer[i][0][j])
                bias_change = (layer[i][1] / propagate) * alpha
                layer[i][1] -= bias_change
                # layer[i][1] = network.betw(layer[i][1])
                layer_weights += (alpha * network.weight_regularization(layer[i][0][j]))
            propagated.append(propagates)
            layer_regularization.append(layer_weights)
            optimizeSAVE.append(optimizeSAVElst)
        return (layer, propagated, layer_regularization, optimizeSAVElst)
    ...
    def direction(prev, nex):
        if nex != 0:
            res = np.sqrt(prev / nex)
            return res
        else:
            return 1
