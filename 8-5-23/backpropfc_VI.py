...
class BPFC:
    ...
    def INIT_PASS(networkFC, init_inputs):
        ...
        return (fc5, fc6, fc7, wr5, wr6, wr7)
    ...
    def FULL(networkFC, init_inputs, fin_outputs, alpha, coords, updated_inputs, overfitted_inputs, prev, loss, step):
        (fc5, wr5) = network.forward_pass(networkFC[1][0], init_inputs, networkFC[1][1], wr = True)
        (fc6, wr6) = network.forward_pass(networkFC[1][1], fc5, networkFC[2], 1, wr = True)
        (fc7, wr7) = network.forward_pass(networkFC[2], fc6, [0, 0], wr = True)
        ...
        bp5 = BPFC.NMBP(networkFC[1][0], bp6)
        filter_loss = bp5
        ...
        rng = np.sqrt(step)
        diff = np.random.rand(4)
        dil = float(40 / float(rng))
        if dil < 1:
            dil = 1
        coor = np.multiply(diff, dil)
        pred_fin = np.multiply(coords, 512)
        for i in range(4):
            if i < 2 and sum(coords) != 0:
                pred_fin[i] += coor[i]
            elif i > 1 and sum(coords) != 0:
                pred_fin[i] -= coor[i]
            elif i < 2:
                val = np.sum(np.random.rand(22))
                if val < 5:
                    pred_fin[i] += coor[i]
            elif i > 1:
                val = np.sum(np.random.rand(22))
            ...
            if pred_fin[i] > 511:
                pred_fin[i] = 511
            pred_fin[i] = int(pred_fin[i])
        err = BPFC.mse(np.multiply(coords, 512), pred_fin)
        filter_matrix = []
        for k in range(8):   
            filter_row = []
            for j in range(8):
        ...
        return (fin_network, err, code, pred_fin, np.multiply(coords, 512))
