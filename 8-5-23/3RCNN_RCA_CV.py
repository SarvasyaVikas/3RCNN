...
from backpropfc_III import BPFC as bp
# Uses DP and MVI
...
class RCNN:
    ...
    def NN(Is1, As1, mult, j, i, alpha, losses, rank):
        start = time.time()
        print(i)
        u = i - mult[j][1]
        section = rank // 16
    ...
    def VR(rank, networkS, As1, u, alpha, losses, sMaps1, sMaps2, sMaps3, sMaps4):
        if (rank % 16) == 0:
            init_inputs = network.connector_layer(sMaps4[(rank):(rank + 16)])
            # init inputs
            (_, fc6, fc7, _, _, _) = bp.INIT_PASS(networkS, init_inputs)
            ...
            (networkS, error, filter_matrix, rho, reverseMatrix, zeros) = cfi.FC(networkS, As1[u][rank // 16], sMaps4, alpha, losses[-1])
