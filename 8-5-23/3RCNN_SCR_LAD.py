...
for loop in range(1):
    ...
    for _ in range(1):
        ...
        for i in range(mult[j][1], mult[j][2] - 4):
            ...
            scr = process.cut_scoring(cut, FOV)
            (h, w) = cut.shape[:2]
            lst.append(h * w)
            CACscore += scr
            print("i")
        ...
        print("perdiff")
        perdiff = 0
        if CACactual != 0 or CACscore != 0:
            perdiff = abs(CACscore - CACactual) / (CACactual + CACscore) * 200
        rec = open("scoresNS2.csv", "a+")
        new = ["NGCT{}".format(mult[j][0]), "SCR_LAD", "P", "-24", rank, CACscore, CACactual, perdiff]
