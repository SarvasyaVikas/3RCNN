...
import os
import csv
from FunctionalNetwork_ROI_LOSS_V import FunctionalNetwork
    ...
    for j in range(len(mult)):
        ...
        #
            ...
            if (rank % 16) == 0:
                ...
                print("fc")
                (networkS, error, filter_matrix, rho, reverseMatrix, zeros, softmax) = FunctionalNetwork.FC(networkS, As1[u][rank // 16], alpha, losses[-1], sMaps4, Is1[u][rank // 16])
                splice_vals = [min(zeros[0], zeros[2]), min(zeros[1], zeros[3]), max(zeros[0], zeros[2]), max(zeros[1], zeros[3])]
                splice_vals = np.array(np.multiply(splice_vals, 64), dtype = np.uint8)
                splices = open("splices.csv", "a+")
                splicewriter = csv.writer(splices)
                splicewriter.writerow(["NGCT{}_ROILOSS".format(mult[j][0]), i, splice_vals[0], splice_vals[1], splice_vals[2], splice_vals[3], As1[u][rank // 16][0], As1[u][rank // 16][1], As1[u][rank // 16][2], As1[u][rank // 16][3], softmax[0], softmax[1]])
                splices.close()
                # modifiers
                # modifiers
                # modifiers
                diffH = splice_vals[3] - splice_vals[1]
                diffW = splice_vals[2] - splice_vals[0]
                delH = diffH // 3
                delW = diffW // 3
                if diffH < 10:
                    splice_vals[3] = min(63, splice_vals[3] + diffH)
                    splice_vals[1] = max(0, splice_vals[1] - diffH)
                if diffH > 30:
                    splice_vals[3] = max(0, splice_vals[3] - delH)
                    splice_vals[1] = min(63, splice_vals[1] + delH)
                if diffW < 10:
                    splice_vals[2] = min(63, splice_vals[2] + diffW)
                    splice_vals[0] = max(0, splice_vals[0] - diffW)
                if diffW > 30:
                    splice_vals[3] = max(0, splice_vals[3] - delW)
                    splice_vals[1] = min(63, splice_vals[1] + delW)
                # modifiers
                # modifiers
                # modifers
                ...
                # end scoring
                for k in range(1, 16):
                    comm.send((networkS, error, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1, rho, reverseMatrix, zeros), dest = rank + k)
            
            if (rank % 16) != 0:
                place = (rank // 16) * 16
                (networkS, error, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1, rho, reverseMatrix, zeros) = comm.recv(source = place)
            print("bp")
