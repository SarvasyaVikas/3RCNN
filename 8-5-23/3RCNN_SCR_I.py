...
for ch in range(56):
    ...
    for _ in range(1):
        ...
        for i in range(mult[j][1], mult[j][2] - 4):
            ...
            scr = process.cut_scoring(cut, FOV, ch + 75)
            (h, w) = cut.shape[:2]
            lst.append(h * w)
            CACscore += scr
            print("i")
        ...
        print("perdiff")
        perdiff = 0
        if CACactual != 0 and CACscore != 0:
            perdiff = abs(CACscore - CACactual) / (CACactual + CACscore) * 200
        tdiff += perdiff
        rec = open("scores.csv", "a+")
        rec1 = open("news.csv", "a+")
        new = ["NGCT{}".format(mult[j][0]), "SCR", "75", rank, CACscore, CACactual, perdiff]
        (csv.writer(rec)).writerow(new)
        rec.close()
        for i in lst:
            new.append(i)
        (csv.writer(rec1)).writerow(new)
        rec1.close()
    print(rank)
    print("end loop") 
    if rank != 0:
        print("send")
        comm.send(tdiff, dest = 0)
    else:
        print("td")
        td = tdiff
        for k in range(1, 65):
            print("recv")
            v = comm.recv(source = k)
            td += v
            print("v")
        ...
        print("t")
        n = open("t.csv", "a+")
        (csv.writer(n)).writerow([ch + 75, td, tot.index(min(tot)), min(tot)])
        n.close()

if rank == 0:
    n = open("news.csv", "a+")
    (csv.writer(n)).writerow(tot)
    w = tot.index(min(tot))
    r = w + 75
    (csv.writer(n)).writerow([r])
    n.close()
