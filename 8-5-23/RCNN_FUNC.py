...
from processes_X import process
...
class RCNN:
    ...
    def VR(rank, networkS, Is1, As1, mult, j, i, alpha, losses, sMaps1, sMaps2, sMaps3, sMaps4, minval, maxval, FOV, CACscore, start, CACactual, scan_start, nn, step, f, art):
        u = i - mult[j][1]
        step += 1
        print("vr start")
        if (rank % 16) == 0:
        ...
        if (rank % 16) == 0:
            ...
            u_inputs = network.connector_layer(uMap4)
            o_inputs = network.connector_layer(oMap4)
            ...
            if rank == 0:
                ...
                splices = open("bb5.csv", "a+")
                splicewriter = csv.writer(splices)
                splicewriter.writerow(["NGCT{}_SLICE{}_{}".format(mult[j][0], i, art), splice_vals[0], splice_vals[1], splice_vals[2], splice_vals[3], coords[0], coords[1], coords[2], coords[3], ann_err]) 
                splices.close()
            if rank != 0:
            ...
            # start scoring
            if rank == 0:   
                score = process.cut_scoring(f[u][0][splice_vals[1]:splice_vals[3], splice_vals[0]:splice_vals[2]], FOV)
                CACscore += score
            # end scoring
