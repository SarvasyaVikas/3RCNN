...
def data(ptn):
    ...
    for i in range(len(rows) - 4):
        ...
        if int(ptn) > 20:
            if sum(vals1) != 0:
                vals1 = np.subtract(vals1, 384)
            if sum(vals2) != 0:
                vals2 = np.subtract(vals2, 384)
            if sum(vals3) != 0:
                vals3 = np.subtract(vals3, 384)
            if sum(vals4) != 0:
                vals4 = np.subtract(vals4, 384)
            if sum(vals5) != 0:
                vals5 = np.subtract(vals5, 384)
            actuals.append([divide(vals1, 768), divide(vals2, 768), divide(vals3, 768), divide(vals4, 768), divide(vals5, 768)])
        else:
        ...
        ol = []
        for k in range(5):
            img = cv2.imread("NGCT{}_IMG/ngct{}_{}.png".format(ptn, ptn, i + k + 1), 0)
            ol.append(img)
        o.append(ol)
        # for k in range(5):
...
for epoch in range(epochs):
    ...
    for j in range(1):
        ...
        for i in range(mult[j][1], mult[j][2] - 4):
            ...
            (csv.writer(sve)).writerow([rank])           
            (csv.writer(sve)).writerow(["VR"])
            (losses, networkS, CACscore, step) = RCNN.VR(rank, networkS, Is1, As1, mult, j, i, alpha, losses, sMap1, sMap2, sMap3, sMap4, minval, maxval, FOV, CACscore, start, CACactual, scan_start, nn, step, f)
