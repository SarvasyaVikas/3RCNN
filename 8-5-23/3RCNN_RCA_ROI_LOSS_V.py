    ...
    for j in range(len(mult)):
        ...
        #
            ...
            if (rank % 16) == 0:
                ...
                print("fc")
                (networkS, error, filter_matrix, rho, reverseMatrix, zeros) = FunctionalNetwork.FC(networkS, As1[u][rank // 16], alpha, losses[-1], sMaps4, Is1[u][rank // 16])
                splice_vals = np.array(np.multiply(zeros, 128), dtype = np.uint8)
                # start scoring
                if rank == 0:	
                	score = process.cut_scoring(Is1[u][rank // 16][splice_vals[1]:splice_vals[3], splice_vals[0]:splice_vals[2]], minval, FOV)
                	CACscore += score
                # end scoring
                for k in range(1, 16):
