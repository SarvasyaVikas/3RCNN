...
class FunctionalNetwork:
    ...
    def recur(val, up, down):
        if (val == 0) or (val == 1):
            return val
        elif (val < down) or (val > up):
            timesten = val * 10
            rem = timesten - int(timesten)
            fin = FunctionalNetwork.recur(rem, up, down)
            return fin
        else:
            return val
    ...
    def extreme(lst):
        ...
        for i in range(len(lst)):
            newval = FunctionalNetwork.recur(lst[i], upper, lower)
    ...
    def FC(networkNS, actual, alpha, prev, sMaps4, image):
        ...
        # zeros = FunctionalNetwork.extreme(zeros)
        ...
        loss = sum(err)
        (networkNS[2], _, _) = network.backprop(networkNS[2], err, 0, alpha, softmax)
        rho = network.direction(prev, loss)
            
        (networkNS[1][1], P6, _) = network.backprop(networkNS[1][1], err, 1, alpha * rho, fcOutput)
        (networkNS[1][0], P5, _) = network.backprop(networkNS[1][0], P6, 0, alpha * rho, fcInput)
        filter_loss = P5
        
        rM = open("reverseMatrices.csv", "a+")
        rMwrite = csv.writer(rM)
        lst = cnInput
        for i in range(4):
            lst.append(actual[i])
        ...
        return (networkNS, loss, filter_matrix, rho, reverseMatrix, zeros, softmax)
