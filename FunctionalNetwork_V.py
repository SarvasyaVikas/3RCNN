...
class FunctionalNetwork:
	...
	def BP(networkNS, error, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1, rank):
		print("4")
		for j in range(4):
			place = (4 * rank) + j
		...
		networkNS[0][1][rank] = optimizerMV.optimize(dMap, sMaps2[rank], apS, networkNS[0][1][rank], alpha * rho)
		
		fMap = network.anti_pool(np.array(dMap), 64, 64)
		eMap = algorithm.convolution(networkNS[0][1][rank], fMap, 2)
			
		networkNS[0][0][rank] = optimizerMV.optimize(eMap, sMaps1[rank], network.anti_pool(sMaps2[rank], 64, 64), networkNS[0][0][rank], alpha * rho)
