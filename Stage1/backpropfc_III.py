import numpy as np
from network_FC_CF_V import network
...
class BPFC:
    ...
    def FULL(networkFC, init_inputs, fin_outputs, alpha, coords, updated_inputs, overfitted_inputs, prev):
        (fc5, wr5) = network.forward_pass(networkFC[1][0], init_inputs, networkFC[1][1], wr = True)
        (fc6, wr6) = network.forward_pass(networkFC[1][1], fc5, networkFC[2], 1, wr = True)
        (fc7, wr7) = network.forward_pass(networkFC[2], fc6, [0, 0], wr = True)
	...
	reverseMatrix = optimizerDP.reverseDense(fin_network[1], pred_fin)

        return (fin_network, min(tse1, tse2, tse3), filter_matrix, rho, reverseMatrix)
