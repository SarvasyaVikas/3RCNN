...
class SNN:
    ...
    def states(images, featureMaps): # applies events with states
        ...
        sAB = techniques.s(images[0], images[1])
        sAC = techniques.s(images[0], images[2])
        sAD = techniques.s(images[0], images[3])
        sAE = techniques.s(images[0], images[4])
        sBC = techniques.s(images[1], images[2])
        sBD = techniques.s(images[1], images[3])
        sBE = techniques.s(images[1], images[4])
        sCD = techniques.s(images[2], images[3])
        sCE = techniques.s(images[2], images[4])
        sDE = techniques.s(images[3], images[4])
        
        forwardPass = []
        
        # recreates forward pass w/ coefficients
        # have to divide by sum of coefficients at the end
        BsAB = np.multiply(featureMaps[1], sAB)
        CsAC = np.multiply(featureMaps[2], sAC)
        DsAD = np.multiply(featureMaps[3], sAD)
        EsAE = np.multiply(featureMaps[4], sAE)
           
        totA = SNN.add([featureMaps[0], BsAB, CsAC, DsAD, EsAE])
        sumA = 1 + sAB + sAC + sAD + sAE
        denomA = 1.0 / sumA
        finA = np.multiply(totA, denomA)
        forwardPass.append(finA)
           
        AsAB = np.multiply(featureMaps[0], sAB)
        CsBC = np.multiply(featureMaps[2], sBC)
        DsBD = np.multiply(featureMaps[3], sBD)
        EsBE = np.multiply(featureMaps[4], sBE)
           
        totB = SNN.add([AsAB, featureMaps[1], CsBC, DsBD, EsBE])
        sumB = sAB + 1 + sBC + sBD + sBE
        denomB = 1.0 / sumA
        finB = np.multiply(totB, denomB)
        forwardPass.append(finB)
           
        AsAC = np.multiply(featureMaps[0], sAC)
        BsBC = np.multiply(featureMaps[1], sBC)
        DsCD = np.multiply(featureMaps[3], sCD)
        EsCE = np.multiply(featureMaps[4], sCE)
          
        totC = SNN.add([AsAC, BsBC, featureMaps[2], DsCD, EsCE])
        sumC = sAC + sBC + 1 + sCD + sCE
        denomC = 1.0 / sumC
        finC = np.multiply(totC, denomC)
        forwardPass.append(finC)
           
        AsAD = np.multiply(featureMaps[0], sAD)
        BsBD = np.multiply(featureMaps[1], sBD)
        CsCD = np.multiply(featureMaps[2], sCD)
        EsDE = np.multiply(featureMaps[4], sDE)
           
        totD = SNN.add([AsAD, BsBD, CsCD, featureMaps[3], EsDE])
        sumD = sAD + sBD + sCD + 1 + sDE
        denomD = 1.0 / sumD
        finD = np.multiply(totD, denomD)
        forwardPass.append(finD)
           
        AsAE = np.multiply(featureMaps[0], sAE)
        BsBE = np.multiply(featureMaps[1], sBE)
        CsCE = np.multiply(featureMaps[2], sCE)
        DsDE = np.multiply(featureMaps[3], sDE)
           
        totE = SNN.add([AsAE, BsBE, CsCE, DsDE, featureMaps[4]])
        sumE = sAE + sBE + sCE + sDE + 1
        denomE = 1.0 / sumE
        finE = np.multiply(totE, denomE)
        forwardPass.append(finE)
