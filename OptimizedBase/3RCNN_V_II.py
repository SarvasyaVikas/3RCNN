...
import os
import csv
from FunctionalNetwork_V import FunctionalNetwork
...
for i in range(val):
    ...
    if (rank % 16) == 0:
        ...
        print("fc")
        (networkS, error, filter_matrix) = FunctionalNetwork.FC(networkS, As1[i][rank // 16], alpha, losses[-1], sMaps4)
        
        for k in range(1, 16):
            comm.send((networkS, error, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1), dest = rank + k)
        
    if (rank % 16) != 0:
        place = (rank // 16) * 16
        (networkS, error, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1) = comm.recv(source = place)
    print("bp")
    (networkS, error) = FunctionalNetwork.BP(networkS, error, alpha, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1, rank)
    #
    if rank != 0:
