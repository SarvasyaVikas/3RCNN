...
import os
import csv
from FunctionalNetwork_ROI_LOSS import FunctionalNetwork
    ...
    for j in range(len(mult)):
        ...
        #
            ...
            if (rank % 16) == 0:
                ...
                print("fc")
                (networkS, error, filter_matrix, rho, reverseMatrix, A8, A16, A32, A64) = FunctionalNetwork.FC(networkS, As1[u][rank // 16], alpha, losses[-1], sMaps4, Is1[u][rank // 16])
            
                for k in range(1, 16):
                    comm.send((networkS, error, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1, rho, reverseMatrix, A8, A16, A32, A64), dest = rank + k)
            
            if (rank % 16) != 0:
                place = (rank // 16) * 16
                (networkS, error, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1, rho, reverseMatrix, A8, A16, A32, A64) = comm.recv(source = place)
            print("bp")
