...
for epoch in range(epochs):
    ...
    for j in range(len(mult)):
    	(minval, maxval) = process.minmaxscan(mult[j][0])
        print(mult[j][0])
        print("losses")
        losses = [(10 ** 5), (10 ** 5)]
        try:
        ...
        #
            ...
            if (rank % 16) == 0:
                ...
                print("fc")
                (networkS, error, filter_matrix, rho, reverseMatrix, A8, A16, A32, A64, zeros) = FunctionalNetwork.FC(networkS, As1[u][rank // 16], alpha, losses[-1], sMaps4, Is1[u][rank // 16])
                # start scoring
                if rank == 0:	
                	score = process.cut_scoring(Is1[u][rank // 16][zeros[1]:zeros[3], zeros[0]:zeros[2]], minval, FOV)
                	CACscore += score
                # end scoring
                for k in range(1, 16):
                    comm.send((networkS, error, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1, rho, reverseMatrix, A8, A16, A32, A64, zeros), dest = rank + k)
            
            if (rank % 16) != 0:
                place = (rank // 16) * 16
                (networkS, error, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1, rho, reverseMatrix, A8, A16, A32, A64, zeros) = comm.recv(source = place)
            print("bp")
            (networkS, error) = FunctionalNetwork.BP(networkS, error, alpha, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1, rank, rho, reverseMatrix, A8, A16, A32, A64)
        #
            if rank != 0:
	...
	mserror = ((CACactual - CACscore) / CACactual) ** 2
	caclst = ["NGCT{}".format(mult[j][0]), CACscore, CACactual, mserror, abs(CACactual - CACscore)]
