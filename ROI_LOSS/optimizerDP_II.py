...
class optimizerDP:
    ...
    def reverseConvolutionalMerged(sMaps1, sMaps2, sMaps3, sMaps4, reverseMatrix, A8, A16, A32, A64, loss):
        r4 = []
        for i in range(4):
            r = algorithm.anticonvolution(reverseMatrix, cM.networkMerge(A8, sMaps4[i], loss), 1)
            r4.append(r)
        
        g3 = [network.anti_pool(np.add(r4[0], r4[1]), 16, 16), network.anti_pool(np.add(r4[2], r4[3]), 16, 16)]
        r3 = []
        for i in range(2):
            r = algorithm.anticonvolution(g3[i], cM.networkMerge(A16, sMaps3[i], loss), 1)
            r3.append(r)
        
        g2 = network.anti_pool(np.add(r3[0], r3[1]), 32, 32)
        r2 = algorithm.anticonvolution(g2, cM.networkMerge(A32, sMaps2, loss), 2)
        
        g1 = network.anti_pool(r2, 64, 64)
        r1 = algorithm.anticonvolution(g1, cM.networkMerge(A64, sMaps1, loss), 2)
