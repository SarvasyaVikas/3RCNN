...
if (rank % 16) == 0:
    ...
    for i in range(1, 16):
        comm.send(networkS, dest = rank + i)

else:
    place = rank - (rank % 16)
    networkS = comm.recv(source = place)
print("networkS")
...
for i in range(val):
    ...
    print("bp")
    (networkS, error) = FunctionalNetwork.BP(networkS, As1[i][section], alpha, losses[-1], sMaps4, sMaps3, sMaps2, sMaps1, rank)
    #
    if rank != 0:
    ...
    if rank == 0:
        ...
        for k in range(5):
            networkFULL[k][0][0][0] = planeRecurrence[k]
    ...
    if rank == 0:
        ...
        for k in range(48, 64):
            (pRA, pRB, pRC, pRD) = comm.recv(source = k)
            placeA = 4 * (k - 48)
            placeB = placeA + 1
            placeC = placeB + 1
            placeD = placeC + 1
