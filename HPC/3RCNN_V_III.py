...
for i in range(val):
    ...
    section = rank // 16
    rem = rank % 16
    fMap1 = parallel.generate_feature_map(networkS[0][0][rem], Is1[i][section])
    ...
    if (rank % 16) == 0:
        ...
        print("fc")
        print(len(sMaps4))
        (networkS, error, filter_matrix, rho) = FunctionalNetwork.FC(networkS, As1[i][rank // 16], alpha, losses[-1], sMaps4)
    ...
    if rank == 0:
        for n in range(5):
            save(networkFULL[n], "SCAN1_ATT1_RANK{}_PLACE{}_SECTION_{}".format(rank, i, n), losses)
    print("save")
