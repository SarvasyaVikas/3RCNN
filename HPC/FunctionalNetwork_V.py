...
class FunctionalNetwork:
	...
	def FC(networkNS, actual, alpha, prev, sMaps4):
		...
		return (networkNS, error, filter_matrix)
		
	def BP(networkNS, error, alpha, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1, rank):
		print("4")
                sect = rank // 16
                mod = rank % 16
		for j in range(4):
			...
			networkNS[sect][3][place] = network.add([networkNS[sect][3][place], delta])
		cRow = []
		print("3")
		for j in range(2):
			...
			avg = network.multiply(network.add([networkNS[sect][3][filter_place1], networkNS[sect][3][filter_place2]]), 0.5)
			...
			networkNS[sect][2][place] = optimizerMV.optimize(cMap, sMaps3[place], apS, networkNS[sect][2][place], alpha * rho) # OPTIMIZER
			cRow.append(cMap)
		print("12")
		...
		avg = network.multiply(network.add([networkNS[sect][2][filter_place1], networkNS[sect][2][filter_place2]]), 0.5)
		...
		networkNS[sect][1][mod] = optimizerMV.optimize(dMap, sMaps2[mod], apS, networkNS[sect][1][mod], alpha * rho)
		
		fMap = network.anti_pool(np.array(dMap), 64, 64)
		eMap = algorithm.convolution(networkNS[sect][1][mod], fMap, 2)
			
		networkNS[sect][0][mod] = optimizerMV.optimize(eMap, sMaps1[mod], network.anti_pool(sMaps2[mod], 64, 64), networkNS[sect][0][mod], alpha * rho)
