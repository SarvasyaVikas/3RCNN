...
class RCNN:
    ...
    def NN(networkS, Is1, As1, mult, j, i, alpha, losses, rank):
        ...
        if (rank % 16) == 0:
            ...
            print("fc")
            for k in range(1, 16):
                comm.send((sMaps1, sMaps2, sMaps3, sMaps4), dest = rank + k)
        else:
            (sMaps1, sMaps2, sMaps3, sMaps4) = comm.recv(source = (rank - rank % 16))
        
        return (sMaps1, sMaps2, sMaps3, sMaps4)

            
    def VR(rank, networkS, Is1, As1, mult, j, i, alpha, losses, minval, maxval, FOV, CACscore, start, CACactual, scan_start, nn, step, f, art):
        (sMaps1, sMaps2, sMaps3, sMaps4) = RCNN.NN(networkS, Is1, As1, mult, j, i, alpha, losses, rank)
        u = i - mult[j][1]
        step += 1
        print("vr start")
        if (rank % 16) == 0:
            ...
            print("bp alt f")
            #for n in range(1, 16):
            #    comm.send((upd, ovf, Is1, As1, mult, j, i, alpha, losses, wrerr), dest = rank + n, tag = n)

        if (rank % 16) != 0:
            # (upd, ovf, Is1, As1, mult, j, i, alpha, losses, wrerr) = comm.recv(source = (rank - (rank % 16)), tag = (rank % 16))
            print("rcnn nn non 0")
            # (_, _, _, _) = RCNN.NN(upd, Is1, As1, mult, j, i, alpha, losses, rank)
            # (_, _, _, _) = RCNN.NN(ovf, Is1, As1, mult, j, i, alpha, losses, rank)
        ...
        if (rank % 16) == 0:
            ...
            u_inputs = network.connector_layer(sMaps4)
            o_inputs = network.connector_layer(sMaps4)

            (networkS, ann_err, code, zeros, coords) = bp.FULL(orig, init_inputs, fin_outputs, alpha, As1[u][rank // 16], u_inputs, o_inputs, losses[-1], sum(err), step)
            # this has the completed fully connected parts

            st = [orig]
            networkS[0] = st[0][0]
            if rank == 0 and j >= 60:
                splice_vals = zeros.copy()
                splice_vals = np.array(splice_vals, dtype = np.uint16)
                splices = open("LAD_BB_FULL_TESTING_RESULTS_8_SPLIT.csv", "a+")
                splicewriter = csv.writer(splices)
                diff = []
                for pl in range(4):
                    diff.append(coords[pl] - splice_vals[pl])
                splicewriter.writerow(["NGCT{}_SLICE{}_{}".format(mult[j][0], i, art), ann_err, splice_vals[0], splice_vals[1], splice_vals[2], splice_vals[3], coords[0], coords[1], coords[2], coords[3], diff[0], diff[1], diff[2], diff[3]]) 
                splices.close()
            if rank != 0:
            ...
            # start scoring
            if rank == 0 and j >= 60:   
                scr = process.cut_scoring(f[u][0][splice_vals[1]:splice_vals[3], splice_vals[0]:splice_vals[2]], FOV)
                score = scr / 4.0
                CACscore += score
            # end scoring

            for ae in range(1, 16):
                comm.send(ann_err, dest = rank + ae, tag = ae)
        else:
            ann_err = comm.recv(source = rank - (rank % 16), tag = (rank % 16))
        ...
        if rank == 0 and i == mult[j][2] - 5:
            ...
            caclst = ["NGCT{}_{}".format(mult[j][0], art), "CAC", CACscore, CACactual, mserror, abs(CACactual - CACscore)]
            scoresfile = open("LAD_SCORES_FULL_20_RESULTS_8_SPLIT.csv", "a+")
            scoreswriter = csv.writer(scoresfile)
            print("write")
            scoreswriter.writerow(caclst)
            scoresfile.close()

        return (losses, networkS, CACscore, step)
