...
import os
import csv
from FunctionalNetwork_V_I import FunctionalNetwork
...
for j in range(len(mult)):
    ...
    #
        ...
        if (rank % 16) == 0:
            ...
            print("fc")
            (networkS, error, filter_matrix, rho) = FunctionalNetwork.FC(networkS, As1[u][rank // 16], alpha, losses[-1], sMaps4)
        ...
        print("bp")
        (networkS, error) = FunctionalNetwork.BP(networkS, error, alpha, filter_matrix, sMaps4, sMaps3, sMaps2, sMaps1, rank, rho)
    #
        if rank != 0:
