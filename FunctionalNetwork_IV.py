...
class FunctionalNetwork:
	...
	def BP(networkNS, actual, alpha, prev, sMaps4, sMaps3, sMaps2, sMaps1):
		...
		for j in range(len(networkNS[0][2])):
			...
			networkNS[0][2][j] = optimizerMV.optimize(cMap, sMaps3[j], apS, networkNS[0][2][j], alpha * rho) # OPTIMIZER
			cRow.append(cMap)
		print("12")
		for j in range(len(networkNS[0][1])):
			...
			networkNS[0][1][j] = optimizerMV.optimize(dMap, sMaps2[j], apS, networkNS[0][1][j], alpha * rho)
			
			fMap = network.anti_pool(np.array(dMap), 64, 64)
			eMap = algorithm.convolution(networkNS[0][1][j], fMap, 2)
			
			networkNS[0][0][j] = optimizerMV.optimize(eMap, sMaps1[j], network.anti_pool(sMaps2[j], 64, 64), networkNS[0][0][j], alpha * rho)
